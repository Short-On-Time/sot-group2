...
class AdminAddRemedys extends React.Component {
	...
	componentDidMount() {
		...
		if (!urlParams.has("new")) {
			window.location.replace(`/admin/remedy_list`);
		} else {
			this.setState({
				name: "",
				ailment: "",
	...
	makeIt(e) {
		...
		if (toSubmit.title && toSubmit.definition && toSubmit.usage) {
			axios.post(`http://localhost:${config.server_port}/api/admin/add_remedy`, toSubmit);
		} else {
		...
		return (
			<div className="App">
				...
				<div className="site-wrap">
					...
					<h1>Add Remedy</h1>
					<a href="/admin/remedy_list" className="btn btn-primary text-white px-4">go back</a>
					<p />
